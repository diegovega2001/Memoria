...
class DataFrameMaker:
    ...
    def load_metadata(self):
        ...
        model_names = [str(x[0][0]) if len(x) > 0 and len(x[0]) > 0 else '' 
                       for x in make_model_mat['model_names']]
    ...
    def build_dataset(self):
        ...
        print(f"Dataset final: {len(self.dataset_df)} imágenes")
        print(f"Distribución de viewpoints: {self.dataset_df['viewpoint'].value_counts().to_dict()}")
